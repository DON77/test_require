 ...
 *
 */
function requine(target, options) {
    /** Reduction of parameters */
    var caller = getCallerPath();
    var targetPath = getTargetPath({ target: target, caller: caller });
    var config = getConfig(options);
    /** Require modules from target directory. */
    var modulesData = getModulesData(targetPath, config, target);
    /** Combine all required modules to one object. */
    var modulesHash = getModulesHash(modulesData, config);
    /** Export root properties of `imports` object via caller-module instance. */
    exportModules({ modulesHash: modulesHash, target: target, config: config });
    /** Return single object of imported and resolved modules. */
    return modulesHash;
}
exports["default"] = requine;
/**
 ...
 */
function getTargetPath(_a) {
    var target = _a.target, caller = _a.caller;
    ...
    if (is(target, 'Module')) {
        return path.dirname(target.filename);
    }
    /** Get directory path from string if `target` is string of path. */
    if (is(target, 'string') && target.length > 0) {
        var targetResolved = path.resolve(path.dirname(caller), target);
        if (is(targetResolved, 'dir')
            || is(targetResolved, 'file')) {
...
function getConfig(options) {
    ...
    var config = is(options, 'object') ? __assign({}, options) : {};
 ...
 * @returns {typeObjects}
 */
function getModulesData(targetPath, config, target, props) {
    if (props === void 0) { props = []; }
    /** Get files and folders list from current directory otherwise use filepath. */
    var items = is(targetPath, 'dir')
        ? fs.readdirSync(targetPath)
        : [targetPath];
    /** Prepare storage for imported modules */
    var modulesData = [];
    /** Process each item in directory. */
    Promise.all([items.forEach(function (pathname) {
            new Promise(function (resolve, reject) {
                return __awaiter(this, void 0, void 0, function () {
                    var itempath, exports, moduleData;
                    return __generator(this, function (_a) {
                        switch (_a.label) {
                            case 0:
                                itempath = path.resolve(targetPath, pathname);
                                ...
                                if (config.exclude.self === true
                                    && require.main.filename === itempath) {
                                    return [2 /*return*/, void 0];
                                }
                                /** Exclude caller-module file. */
                                if (is(target, 'Module')
                                    && target.filename === itempath) {
                                    return [2 /*return*/, void 0];
                                }
                                /** Exclude parent modules. */
                                if (config.exclude.parents === true
                                    && getParents(module).includes(itempath)) {
                                    return [2 /*return*/, void 0];
                                }
                                /** Drilldown if current item is directory. */
                                if (is(itempath, 'dir')
                                    && config.recurse === true) {
                                    return [2 /*return*/, getModulesData(itempath, config, target, props.concat([pathname]))];
                                }
                                exports = require(itempath);
                                ...
                                return [4 /*yield*/, modulesData.push(moduleData)];
                            case 1:
                                /** Save module data to common array. */
                                _a.sent();
                                resolve(modulesData);
 ...
 * @returns {Object}
 */
function getModulesHash(modulesData, config) {
    ...
    if (Array.isArray(modulesData)) {
        modulesData.forEach(function (_a) {
            var props = _a.props, filename = _a.filename, basename = _a.basename, modulePath = _a.path, extname = _a.extname, moduleExports = _a.exports;
            /** Get object property path. */
            var propsPath = (basename === config.indexName
            ...
            var indexPath = path.join(path.dirname(modulePath), "/" + config.indexName + extname);
            if (config.indexProp === false
                && basename !== config.indexName
                && config.exclude.siblings === true
                && modulesData.some(function (_a) {
                    var path = _a.path;
            ...
            if (objectPath.has(modulesHash, propsPath)
                && config.indexProp === false) {
                /** Get source prop from Imports. */
                var moduleData = objectPath.get(modulesHash, propsPath);
                /** Reassign source prop if. */
                moduleData = basename === config.indexName
                    ? Object.assign(moduleExports, moduleData)
                    : Object.assign(moduleData, (_b = {}, _b[basename] = moduleExports, _b));
 ...
 * @return {void}
 * */
function setObjectPath(modulesHash, propsPath, moduleExports) {
    propsPath.length > 0
 ...
 */
function exportModules(_a) {
    var modulesHash = _a.modulesHash, target = _a.target, config = _a.config;
    /** Effective exporter function. */
    var exporter = is(config["export"], 'function')
        ? config["export"]
        : exporterByDefault;
     ...
     * */
    function isExportableData() {
        var keys = Object.keys(modulesHash);
        return (
        ...
        is(target, 'Module')
            ...
            && keys.some(function (key) { return !is(key, 'undefined'); })
            /** Export is allowed. */
     ...
     * */
    function isExportableItem(moduleName, modulesHash) {
        return (
        /** Key is string. */
    ...
    function exportEach() {
        for (var moduleName in modulesHash) {
            /** Get exports of module by it name from `modulesHash`. */
            var moduleExports = modulesHash[moduleName];
            ...
            if (isExportableItem(moduleName, modulesHash)) {
                exporter({ moduleName: moduleName, moduleExports: moduleExports, modulesHash: modulesHash, target: target });
            }
        }
    }
    /** Just export current value by default. */
    function exporterByDefault(_a) {
        var moduleName = _a.moduleName, moduleExports = _a.moduleExports, modulesHash = _a.modulesHash, target = _a.target;
        target.exports[moduleName] = this;
    }
}
 ...
 * */
function getSafePropName(propName, obj) {
    /** Uniquely safe `moduleName` by sequential index in the context of `modulesHash`. */
    var safePropName = "" + propName + this.moduleNameIndex;
    this.index = this.index || 1;
    this.index++;
    var uniqueVarName = obj.hasOwnProperty(propName)
